# Cursor CLI Stream Deck integration

[Project overview](../../README.md) · [Documentation index](../README.md)

Use Cursor CLI with a physical Stream Deck status key and Windows window switching. Install the broker once and this profile in each project you want to monitor. OpenCode is not a prerequisite.
...
cd C:\Projects\MyApp
python -m ocdeck harness-install cursor --dry-run
python -m ocdeck harness-install cursor
agent
```

The installer merges `.cursor/hooks.json` and records ownership in `.agentdeck/cursor.json`. It preserves unrelated hooks/settings and writes a backup before changing an existing config. JSONC requires manual reconciliation; the installer refuses to silently strip comments. Restart an already-running harness so it loads new hooks.

## What the key tells you

The executable is `agent`. This profile targets Cursor CLI, not the Cursor desktop application. It reports activity without guaranteed approval counts; a busy key is not proof that no interaction is required.
...
```powershell
python -m ocdeck harness-install cursor --project C:\Projects\MyApp --remove --dry-run
python -m ocdeck harness-install cursor --project C:\Projects\MyApp --remove
```

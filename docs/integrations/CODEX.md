# Codex CLI Stream Deck integration

[Project overview](../../README.md) · [Documentation index](../README.md)

Use Codex CLI with a physical Stream Deck status key and Windows window switching. Install the broker once and this profile in each project you want to monitor. OpenCode is not a prerequisite.
...
cd C:\Projects\MyApp
python -m ocdeck harness-install codex --dry-run
python -m ocdeck harness-install codex
codex
```

The installer merges `.codex/hooks.json` and records ownership in `.agentdeck/codex.json`. It preserves unrelated hooks/settings and writes a backup before changing an existing config. JSONC requires manual reconciliation; the installer refuses to silently strip comments. Restart an already-running harness so it loads new hooks.

## What the key tells you

Review and trust the installed entries with `/hooks`. PermissionRequest sets INPUT with an unknown count. It is observed approval state without a paired approval-resolution ID. A subsequent result, new turn, stop or interrupt can clear it. Paired `request_user_input` tool IDs can be tracked. The adapter never changes hook trust or approval policy.
...
```powershell
python -m ocdeck harness-install codex --project C:\Projects\MyApp --remove --dry-run
python -m ocdeck harness-install codex --project C:\Projects\MyApp --remove
```

# GitHub Copilot CLI Stream Deck integration

[Project overview](../../README.md) · [Documentation index](../README.md)

Use GitHub Copilot CLI with a physical Stream Deck status key and Windows window switching. Install the broker once and this profile in each project you want to monitor. OpenCode is not a prerequisite.
...
cd C:\Projects\MyApp
python -m ocdeck harness-install copilot-cli --dry-run
python -m ocdeck harness-install copilot-cli
copilot
```

The installer merges `.github/hooks/agentdeck-copilot-cli.json` and records ownership in `.agentdeck/copilot-cli.json`. It preserves unrelated hooks/settings and writes a backup before changing an existing config. JSONC requires manual reconciliation; the installer refuses to silently strip comments. Restart an already-running harness so it loads new hooks.

## What the key tells you

This adapter reports activity only where the native payload does not provide paired request IDs. A busy key can still correspond to an approval wait. Inspect the Copilot terminal when progress pauses.
...
```powershell
python -m ocdeck harness-install copilot-cli --project C:\Projects\MyApp --remove --dry-run
python -m ocdeck harness-install copilot-cli --project C:\Projects\MyApp --remove
```

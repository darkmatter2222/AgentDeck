...
Use Claude Code with a physical Stream Deck status key and Windows window switching. Install the broker once and this profile in each project you want to monitor. OpenCode is not a prerequisite.
...
cd C:\Projects\MyApp
python -m ocdeck harness-install claude --dry-run
python -m ocdeck harness-install claude
claude
```

The installer merges `.claude/settings.local.json` and records ownership in `.agentdeck/claude.json`. It preserves unrelated hooks/settings and writes a backup before changing an existing config. JSONC requires manual reconciliation; the installer refuses to silently strip comments. Restart an already-running harness so it loads new hooks.

## What the key tells you

Paired `AskUserQuestion` tool-use IDs can establish INPUT until result or turn end. Unpaired PermissionRequest events and notifications produce UNKNOWN rather than a made-up pending total. Keep the terminal visible when resolving permissions.

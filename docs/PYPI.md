# Publish AgentDeck to PyPI

The repository is prepared for keyless publishing. Account-side PyPI authorization
must be configured by someone with access to the PyPI account. The current
`agentdeck` PyPI JSON endpoint returned 404 when checked on 2026-09-12; this does
not reserve the name or guarantee that PyPI will permit registration.
...
3. Open [GitHub repository Environments](https://github.com/darkmatter2222/AgentDeck/settings/environments).
...
4. Open [Signed Python release](https://github.com/darkmatter2222/AgentDeck/actions/workflows/pypi.yml),
...
6. Verify [the project on PyPI](https://pypi.org/project/agentdeck/2.1.0/), then install:

   ```powershell
   python -m pip install --upgrade agentdeck==2.1.0
   ocdeck --version
   ```
...
If **you own** `agentdeck`, use that project's **Manage → Publishing** page and
...
- **Invalid publisher / token exchange rejected:** compare all five values exactly;
  the workflow must run from `main` in `darkmatter2222/AgentDeck`, in environment `pypi`.

...
class CustomizationTests(unittest.TestCase):
    ...
    def test_alias_replaces_project_but_never_status(self):
        ...
        self.assertEqual(frame('input','Original',0,24,80,a).tobytes(),frame('input','Reviewer',0,24,80,replace(a,alias='')).tobytes())
        self.assertNotEqual(frame('input','Original',0,24,80,a).tobytes(),frame('idle','Original',0,24,80,a).tobytes())

    def test_text_effects_are_clipped_and_change_pixels(self):
        for effect in ('scroll','shimmer'):
            a=Appearance(layout='harness',intensity=0,primary='alias',alias='Long project alias for scrolling',text_effect=effect)
    ...
    def test_new_choices_and_presets_render(self):
        for field,values in {'text_size':['small','normal','large'],'text_align':['left','center','right'],
          'badge':['dot','ring','pill'],'border':['solid','double','corners','none'],
          'background':['solid','gradient','grid'],'logo_size':['small','normal','large']}.items():
            for value in values:
                ...
                for state in ('running','idle','input','unknown','ready','off'):
                    ...
                    else: self.assertIsNotNone(im.getbbox())
            with self.assertRaises(ValueError): appearance({'appearance':{field:'bogus'}})
        for options in PRESETS.values(): appearance({'appearance':options})
        with self.assertRaises(ValueError): appearance({'appearance':{'alias':'x'*101}})
        with self.assertRaises(ValueError): appearance({'appearance':{'theme':[]}})

    def test_cli_presets_override_precedence_and_preserve_labels(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);env=dict(os.environ,OCDECK_HOME=td)
            def cli(*args):
                return subprocess.run([sys.executable,'-m','ocdeck',*args],env=env,capture_output=True,text=True)
            p=root/'config.json'
            p.write_text(json.dumps({'serial':'KEEP','appearance':{'alias':'Main'},'buttons':{'2':{'alias':'Review','custom_text':'Keep'}}}))
            ...
            self.assertEqual(p.read_bytes(),before)
            with Image.open(output) as im: self.assertGreater(im.width,0)

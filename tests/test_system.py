...
class ModelTests(unittest.TestCase):
    ...
    def test_colors_and_input_priority(self):
        ...
        self.assertEqual(self.r.view()[0]["state"], "running")
        self.state(status="idle", pending=2, seq=2)
        self.assertEqual(self.r.view()[0]["state"], "input")
        self.state(status="idle", pending=1, seq=3)
        self.assertEqual(self.r.view()[0]["state"], "input")
        self.state(status="idle", pending=0, seq=4)
        self.assertEqual(self.r.view()[0]["state"], "idle")
...
class ArtTests(unittest.TestCase):
    ...
    def test_press_callback_only_uses_down_and_rendered_generation(self):
        ...
        d.press(0, True)
        self.assertEqual(q.get()["generation"], 3)
...
class HTTPTests(unittest.TestCase):
    def setUp(self):
        ...
        self.identity_patch = patch("ocdeck.broker.identity", identity)
        self.identity_patch.start()
        self.focused = []
        self.b = Broker(self.root, mock=True, focus=lambda r: self.focused.append(r["id"]) or {"ok": True})
        self.thread = threading.Thread(target=self.b.serve)
        self.thread.start()
        for _ in range(100):
            if (self.root / "discovery.json").exists():
                break
            time.sleep(0.01)
    ...
    def api(self, method, path, body=None):
        return request(method, path, body, root=self.root)
    ...
    def test_dead_real_process_removed(self):
        ...
        try:
            ...
            child.wait()
            for _ in range(30):
                if self.api("GET", "/v1/status")["slots"][0]["state"] == "off":
                    break
                time.sleep(0.1)
            self.assertEqual(self.api("GET", "/v1/status")["slots"][0]["state"], "off")
        finally:
    ...
    def test_pid_reuse_wrong_creation_time_is_rejected(self):
        ...
        with self.assertRaises(urllib.error.HTTPError):
            self.api("POST", "/v1/register", {"id": "bad", "process": p})

    def test_ready_only_in_device_frame_and_relinquishes_slot(self):
        for _ in range(20):
            if self.b.device.presented[0]:
                break
            time.sleep(0.05)

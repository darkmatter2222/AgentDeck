...
if not binary:
    raise SystemExit("Set OPENCODE_TEST_BIN first; this is an optional live integration test.")
                    ...
                    "questions": [
                        {
                            ...
                            "header": "Deck test",
                            "options": [{"label": "Continue", "description": "Complete the deterministic fixture"}],
                        }
                    ]
                }
                if tool == "question"
                else {"command": "echo DECK_TEST", "description": "Harmless Stream Deck fixture"}
            )
            delta = {
                "tool_calls": [
                    {
...
with tempfile.TemporaryDirectory() as td:
    ...
    atomic_json(binding, {"id": "live-runtime", "process": identity(), "label": "OpenCode live", "managed": True})
    ...
    with patch("ocdeck.broker.identity", identity):
        ...
        env = dict(
            os.environ,
            OCDECK_HOME=str(root),
            OCDECK_BINDING=str(binding),
        ...
        process = subprocess.Popen(
            [binary, "serve", "--hostname", "127.0.0.1", "--port", str(port)], env=env, cwd=root, stdout=log, stderr=log
        )
        ...
        def color():
            return request("GET", "/v1/status", root=root)["slots"][0]["state"]

        try:
            wait(lambda: api("GET", "/global/health"), "OpenCode startup")
            print("Actual OpenCode server started", flush=True)
            for case, route, reply in [
                ("permission-case", "permission", {"reply": "once"}),
                ("question-case", "question", {"answers": [["Continue"]]}),
            ]:

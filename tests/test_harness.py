...
class HarnessIntegrationTests(unittest.TestCase):
    ...
    def state(self, expected):
        for _ in range(200):
            ...
            if value["slots"][0]["state"] == expected:
                return value
            time.sleep(0.025)
    ...
    def hook(self, env, event, payload):
        result = subprocess.run(
            ["node", str(SOURCE / "plugins/harnesses/hook.mjs"), "claude", event],
    ...
    def test_bridge_restart_pending_identity_and_loss(self):
        ...
        descriptor = self.root / "hook.json"
        ...
        bridge = subprocess.Popen(
            ["node", str(SOURCE / "plugins/harnesses/bridge.mjs"), "--worker", "claude", str(binding), str(descriptor)],
            env=env,
        ...
        try:
            for _ in range(100):
                ...
                time.sleep(0.025)
            self.state("unknown")
            self.hook(env, "SessionStart", {"session_id": "s"})
            self.state("idle")
            self.hook(env, "UserPromptSubmit", {"session_id": "s"})
            self.state("running")
            ...
            self.hook(env, "PostToolUseFailure", {"session_id": "s", "tool_use_id": "q"})
            self.state("running")
            self.hook(env, "Stop", {"session_id": "s"})
            self.state("idle")
...
""")
        spec = {
            ...
            "cwd": str(SOURCE),
            "args": [str(fake), "space arg", "a&b", 'quote"value', "$literal"],
            "windowToken": "",
        }
        with (
            patch.dict(
                os.environ, {"OCDECK_HOME": str(self.root), "HOOK_SCRIPT": str(SOURCE / "plugins/harnesses/hook.mjs")}
            ),

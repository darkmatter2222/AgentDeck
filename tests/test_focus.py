...
class Desktop:
    ...
    def GetWindowThreadProcessId(self, hwnd, _):
        return {100: 10, 200: 20}.get(hwnd, 0)

    def IsWindow(self, hwnd):
        return hwnd in (100, 200, 201)

    def IsHungAppWindow(self, hwnd):
        return False

    def IsChild(self, parent, child):
        return parent == 200 and child == 201
    ...
    def GetGUIThreadInfo(self, thread, ptr):
        ptr._obj.hwndFocus = self.focus
        return True

    def SetForegroundWindow(self, hwnd):
        if self.direct or {10, 20} <= self.attached:
    ...
    def AttachThreadInput(self, current, other, attach):
        ...
        if attach and other in self.denied:
            return False
        if attach:
            self.attached.add(other)
        else:
    ...
    def SetFocus(self, hwnd):
        ...
        if self.raise_focus:
            raise OSError("test focus failure")
        if 20 in self.attached:
            self.focus = hwnd
...
class FocusTests(unittest.TestCase):
    def test_attaches_destination_and_foreground_preserves_input_child(self):
        ...
        self.assertTrue(result["keyboardFocus"])
    ...
    def test_direct_success_requires_keyboard_focus(self):
        ...
        result = focus_window(u, 30, 200, sleep=lambda _: None)
        self.assertTrue(result["ok"])
        self.assertIn(("focus", 200), u.events)
    ...
    def test_denied_attachment_does_not_report_success(self):
        ...
        self.assertEqual(result["attachmentFailures"], [20])
        self.assertFalse(u.attached)
    ...
    def test_exception_always_detaches(self):
        ...
        with self.assertRaises(OSError):
            focus_window(u, 30, 200, sleep=lambda _: None)
        self.assertFalse(u.attached)

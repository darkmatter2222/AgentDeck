...
class AppearanceTests(unittest.TestCase):
    def test_overrides_and_validation(self):
        ...
        for value in (float("nan"), float("inf"), -1, 4):
            with self.assertRaises(ValueError):
                appearance({"appearance": {"speed": value}})
        with self.assertRaises(ValueError):
            appearance({"appearance": {"theme": "typo"}})
    ...
    def test_motion_and_brightness(self):
        ...
        dim = frame("running", "Test", 0, 24, 80, Appearance(brightness=0.3))
    ...
    def test_identical_pixels_still_update_focus_assignment(self):
        ...
        try:
            ...
            while not d.presented[0] and time.monotonic() < deadline:
                time.sleep(0.01)
            r.remove("old")
            r.upsert({"id": "new", "process": {"pid": 2}, "label": "Same"})
            deadline = time.monotonic() + 2
            while d.presented[0]["id"] != "new" and time.monotonic() < deadline:
                time.sleep(0.01)
            d.press(0, True)

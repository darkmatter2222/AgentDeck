...
"""
text = re.sub(r"## How it works\n.*?(?=\n## Living Jelly:)", lambda _m: how.rstrip(), text, flags=re.S)

commands = """## Commands and configuration
...
"""
text = re.sub(r"## Commands and configuration\n.*?(?=Broker settings live)", lambda _m: commands, text, flags=re.S)
text = text.replace(
    "Compatibility names remain `ocdeck`, `.opencode-deck` and the\n`OpenCode Deck` scheduled task; renaming the project does not rename installed state.",
    "Compatibility names remain `ocdeck` and the `.opencode-deck` state directory. New Windows startup registration uses the **AgentStreamDeck Broker** Scheduled Task; uninstall also recognizes an owned legacy `OpenCode Deck` task.",
)

uninstall = """## Uninstall and backups
...
"""
text = re.sub(
    r"## Uninstall and backups\n.*?(?=\n## Updates, packaging and release status)",
    lambda _m: uninstall.rstrip(),
    text,
    flags=re.S,
)

updates = """## Updates, packaging and release status
...
"""
text = re.sub(
    r"## Updates, packaging and release status\n.*?(?=\n## Tests and contributor documentation)",
    lambda _m: updates.rstrip(),
    text,
    flags=re.S,
)
